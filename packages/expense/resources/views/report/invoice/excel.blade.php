<div>
    <table>
        <tr>
            <th style="text-align: right; font-size: 16px;" colspan="9">
                <strong> ทบ.อส.1 </strong>
            </th>
        </tr>
        <tr> </tr>
        <tr>
            <th style="text-align: center; font-size: 16px;" colspan="9">
    ...
    <table class="table table-responsive-sm table-bordered" style="border: 1px solid #000000;">
        <thead>
            <tr>
                <th style="border: 1px solid #000000; text-align: center; vertical-align: middle;" rowspan="2">
                    <strong> วันทีเอกสารขอเบิก </strong>
                </th>
                <th style="border: 1px solid #000000; text-align: center; vertical-align: middle;" rowspan="2">
        ...
        <tbody>
            @foreach ($invoices as $index => $invoice)
                ...
                @endphp
                <tr>
                    <td style="border: 1px solid #000000; text-align: center;"> {{ $day.' '.$month.' '.$year }} </td>
                    <td style="border: 1px solid #000000; text-align: center;"> {{ optional($invoice)->req_number }} </td>
                    <td style="border: 1px solid #000000; text-align: left;"> {{ $invoice->supplier_name }} </td>
                    <td style="border: 1px solid #000000; text-align: left;"> {{ $invoice->description }} </td>
                    <td style="border: 1px solid #000000; text-align: right;"> {{ number_format($invoice->amount, 2) }} </td>
                    <td style="border: 1px solid #000000; text-align: left;"> {{ $invoice->user->hrEmployee->full_name }} </td>
                    ...
                    <td style="border: 1px solid #000000; text-align: left;"> {{ $invoice->note }} </td>
                </tr>
            @endforeach
        </tbody>
    </table>
