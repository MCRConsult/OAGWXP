<template>
    <span>
        <button type="button" class="btn btn-block btn-sm btn-danger m-1" data-toggle="collapse" @click.prevent="openModal()">
            ยกเลิก
        </button>
        <div :class="'modal fade modal_cancel'+header.id" aria-labelledby="myModalLabel" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog modal-md">
                <div class="modal-content" v-loading="loading">
                    ...
                    <div class="modal-body text-left" style="padding: 15px;">
                        <form :class="'cancel-form'+header.id" onkeydown="return event.key != 'Enter';">
...
<script>
    ...
    export default {
        ...
        methods: {
            openModal() {
                $('.modal_cancel'+this.header.id).modal('show');
            },
            closeModal() {
                this.reason = '',
                $('.modal_cancel'+this.header.id).modal('hide');
            },
            async setStatus(){
                var vm = this;
                let valid = true;
                let errorMsg = '';
                var form = $('.cancel-form'+this.header.id);
                if (vm.reason == '' || vm.reason == null){
                    vm.errors.reason = true;
                    valid = false;
                    errorMsg = "กรุณาระบุเหตุผลในการยกเลิก";
                ...
                Swal.fire({
                    ...
                    html: 'ต้องการ <b>ยืนยัน</b> ยกเลิกรายการใช่หรือไม่?',
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(function (res) {
                            ...
                            if (res.data.message) {
                                ...
                                vm.reason = '',
                                $('.modal_cancel'+vm.header.id).modal('hide');
                                Swal.fire({
                                    title: 'ยืนยันยกเลิกรายการ',
                                    html: 'ส่งยกเลิกรายการเรียบร้อยแล้ว',
                                    icon: "success",
                                    showCancelButton: false,
                                    confirmButtonColor: "#3085d6",
                                    confirmButtonText: "ตกลง",
